...
function extractContactEmail(values) {
  if (!values || typeof values.email !== 'string') return null
  const email = values.email.trim()
  return email || null
}

async function sendSubmissionNotification(applicationId) {
  const { data: { session } } = await supabase.auth.getSession()
  if (!session?.access_token) throw new Error('No active session for submission notification')

  let lastError = null
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      const response = await fetch(`/api/applications/${applicationId}/submission-notification`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${session.access_token}` },
      })
      const body = await response.json().catch(() => ({}))
      if (response.ok) return body
      lastError = new Error(body.detail || body.error || 'Submission notification failed')
    } catch (error) {
      lastError = error
    }
  }
  throw lastError || new Error('Submission notification failed')
}
...
export function useDraft({ program, user }) {
    ...
    try {
      if (applicationIdRef.current) {
        const { error } = await supabase
          ...
          .update({
            form_data: values,
            contact_email: extractContactEmail(values),
          })
  ...
  const submit = useCallback(
    async (values) => {
      ...
      try {
        let submittedApplicationId = applicationIdRef.current
        if (applicationIdRef.current) {
          const { error } = await supabase
            .from('applications')
            .update({
        ...
        setStatus('saved')
        try {
          await sendSubmissionNotification(submittedApplicationId)
          return { ok: true, notificationSent: true }
        } catch (notificationError) {
          console.error('Application submitted but PDF notification failed', notificationError)
          return {
            ok: true,
            notificationSent: false,
            notificationError: notificationError.message || 'PDF notification failed',
          }
        }
      } catch {
        setStatus('error')
