...
- `mentor_milestone_rates` — **milestone payouts are priced per (mentor assignment, milestone), not a single flat rate.** Two co-mentors on the same student are never split evenly — each has their own independent schedule, and a milestone with no rate row for a given mentor simply isn't payable for them.
- `mentor_payment_records` — the payable ledger, auto-generated by Postgres triggers:
  - Completing a milestone creates a `pending` line for every mentor on that student who has a custom rate configured for that specific milestone; reverting the milestone removes only still-`pending` lines (paid ones are kept for audit).
  - Setting/editing a mentor's rate for a milestone that's already completed backfills or updates the (still-pending) payable line retroactively.
...
- `StudentCourseHours.jsx` class-logging form now selects a real mentor assignment instead of a free-text role.

### Required setup
- Run `docs/sql/migrations/2026-08-13_add_mentor_payments.sql` in Supabase (after the 2026-08-12 migration). Mentor payments won't work until this is applied. (Amended in place rather than stacking a follow-up migration, since it hadn't been applied anywhere yet when the per-milestone pricing model was added.)
- For each mentor assignment: set a payment type, and for `milestone`-type mentors, price each milestone individually (Current students > student > Mentor assignments) — unpriced milestones generate no payout for that mentor.
...
- Not yet verified against a live Supabase instance (migration not applied there) — needs an authenticated walkthrough (assign two mentors to one student with different per-milestone schedules, complete a milestone, confirm each mentor gets their own independently-priced line, not a split) before relying on this in production.
