...
### Background
Internal admin/accounting had no way to see whether mentors had been paid. Class sessions and milestone completions happened in the admin UI but weren't tied to a real mentor record or a payable ledger.

### Added
- `class_sessions.assignment_id` — logging a class now records the actual mentor assignment (replaces the free-text `mentor_role` string match).
- `mentor_payment_settings` — per mentor-assignment payment type (`milestone` or `hourly`); the hourly rate lives here.
- `mentor_milestone_rates` — **milestone payouts are priced per (mentor assignment, milestone), not a single flat rate.** Rates are configured per mentor, but each milestone is only ever paid to whichever single mentor is marked responsible for it (see below) — co-mentors are never split evenly.
- `student_milestone_progress.assignment_id` — **each milestone is delivered by exactly one mentor per student.** Admins pick the responsible mentor per milestone (Current students > student > Course hours > milestone list), independent of and settable before/after milestone status. `set_student_milestone_mentor` RPC handles this (validates the mentor is actually assigned to the student).
- `mentor_payment_records` — the payable ledger, auto-generated by Postgres triggers:
  - Completing a milestone creates a `pending` line for the one responsible mentor, if they're `milestone`-type and have a rate set for that milestone. Reassigning the responsible mentor updates that same line (never a second one — one payment record per milestone completion). Reverting the milestone removes only a still-`pending` line (paid stays for audit).
  - Setting/editing a mentor's rate for a milestone they're already responsible for and that's already completed backfills or updates the (still-pending) payable line retroactively.
  - Logging/editing/deleting a class session creates/updates/removes a `pending` line for the assigned mentor when they're configured as `hourly`-type; already-`paid` lines are never touched automatically.
- `set_mentor_payment_status` RPC — admin-only, marks a record paid/pending and allows editing the amount at pay time.
- New `components/admin/MentorAssignments.jsx` — since the old CSV-import mentor-assignment flow was removed (2026-08-02 session), this is now the only way to add/remove a mentor for a student, set payment type, and price their per-milestone schedule (or hourly rate).
- New top-level **Mentor payments** admin tab (`components/admin/MentorPayments.jsx`) — mentor list with pending/paid totals, per-mentor ledger showing each priced milestone, mark-paid/revert, and manual line-item adjustments.
- `StudentCourseHours.jsx` class-logging form selects a real mentor assignment instead of a free-text role; the milestone list now has a "responsible mentor" select alongside the status select.

### Required setup
- Run `docs/sql/migrations/2026-08-13_add_mentor_payments.sql` in Supabase (after the 2026-08-12 migration). Mentor payments won't work until this is applied. (Amended in place twice rather than stacking follow-up migrations, since it hadn't been applied anywhere yet.)
- Per student: assign a responsible mentor to each milestone (Course hours section), and for that mentor, set payment type + price that milestone (Mentor assignments section) — a milestone with no responsible mentor, or a responsible mentor with no rate for it, never generates a payout.
