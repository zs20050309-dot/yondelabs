...
- `mentor_payment_settings` — per mentor-assignment payment type (`milestone` flat rate or `hourly` rate) and rate, editable from the student's Mentor assignments section.
- `mentor_payment_records` — the payable ledger, auto-generated by Postgres triggers:
  - Completing a milestone creates a `pending` line for every mentor on that student configured as `milestone`-type; reverting the milestone removes only still-`pending` lines (paid ones are kept for audit).
...
- New `components/admin/MentorAssignments.jsx` — since the old CSV-import mentor-assignment flow was removed (2026-08-02 session), this is now the only way to add/remove a mentor for a student and set their payment type/rate.
- New top-level **Mentor payments** admin tab (`components/admin/MentorPayments.jsx`) — mentor list with pending/paid totals, per-mentor ledger, mark-paid/revert, and manual line-item adjustments.
- `StudentCourseHours.jsx` class-logging form now selects a real mentor assignment instead of a free-text role.

### Required setup
- Run `docs/sql/migrations/2026-08-13_add_mentor_payments.sql` in Supabase (after the 2026-08-12 migration). Mentor payments won't work until this is applied.
- For each mentor assignment, an admin must set a payment type + rate (Current students > student > Mentor assignments) before milestone/session logging will generate payable lines for that mentor.
...
- Not yet verified against a live Supabase instance (migration not applied there) — needs an authenticated walkthrough per the steps in the implementation plan before relying on this in production.
